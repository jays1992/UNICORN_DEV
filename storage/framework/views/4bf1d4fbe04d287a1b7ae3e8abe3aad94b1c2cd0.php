      ...
      <div class="container-fluid purchase-order-view filter">     
         <form id="frm_mst_edit" method="POST"  > 
          ...
          <div class="inner-form">
              <div class="row">
                  <div class="col-lg-2 pl"><p>Task Status Code</p></div>
                  <div class="col-lg-2 pl">
                    <label> <?php echo e($objResponse->TASK_STATUSCODE); ?> </label>
                    <input type="hidden" name="TASK_STATUSCODE" id="TASK_STATUSCODE" value="<?php echo e($objResponse->ID); ?>" />
                    <input type="hidden" name="TASK_STATUSCODE" id="TASK_STATUSCODE" value="<?php echo e($objResponse->TASK_STATUSCODE); ?>" autocomplete="off"  maxlength="20"   />
                </div>
                </div>

                <div class="row">
                  <div class="col-lg-2 pl"><p>Task Status Name</p></div>
                  <div class="col-lg-5 pl">
                    <input <?php echo e($ActionStatus); ?> type="text" name="TASK_STATUSNAME" id="TASK_STATUSNAME" class="form-control mandatory" value="<?php echo e(old('TASK_STATUSNAME',$objResponse->TASK_STATUSNAME)); ?>" maxlength="200" tabindex="1"  />
                  </div>
                </div>
  ...
  $('#btnExit').on('click', function() {
    ...
    var formDataMst = $( "#frm_mst_edit" );
     formDataMst.validate();
      $("#TASK_STATUSNAME").blur(function(){
        $(this).val($.trim( $(this).val() ));
        $("#ERROR_DESCRIPTIONS").hide();
        validateSingleElemnet("TASK_STATUSNAME");
      });

    $("#TASK_STATUSNAME").keydown(function(){
      $("#ERROR_DESCRIPTIONS").hide();
      validateSingleElemnet("TASK_STATUSNAME");
      });

      $( "#TASK_STATUSNAME" ).rules( "add", {
        required: true,
...
function submitForm(requestType){
  ...
  $.ajax({
      ...
      success:function(data) {
        ...
        if(data.errors) {
        $(".text-danger").hide();
        if(data.errors.TASK_STATUSNAME){
        showError('ERROR_ACTIVITYNAME',data.errors.TASK_STATUSNAME);
        }
      }
        if(data.success) {                   
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\bsquareappfordemo.com\UNICORN\resources\views\masters\PreSales\TaskStatus\mstfrm438edit.blade.php ENDPATH**/ ?>