...
<div class="container-fluid purchase-order-view filter">     
  <form id="frm_mst_edit" method="POST"> 
    <?php echo csrf_field(); ?>
    <?php echo e(isset($objResponse->EMPHIERCHYID) ? method_field('PUT') : ''); ?>

   <div class="inner-form">
         <div class="row">
           <div class="col-lg-2 pl"><p>Hierarchy Code*</p></div>
           <div class="col-lg-2 pl">
              <input <?php echo e($ActionStatus); ?> type="text" name="DOC_NO" id="DOC_NO" value="<?php echo e(isset($objResponse->EMPHIERCHY_NO) && $objResponse->EMPHIERCHY_NO !=''?$objResponse->EMPHIERCHY_NO:''); ?>"  class="form-control mandatory"  autocomplete="off" readonly >
             <span class="text-danger" id="ERROR_DOC_NO"></span>
           </div>

           <div class="col-lg-2 pl"><p>Document Date*</p></div>
           <div class="col-lg-2 pl">
           <input <?php echo e($ActionStatus); ?> type="date" name="DOC_DT" id="DOC_DT" value="<?php echo e(isset($objResponse->EMPHIERCHY_DATE) && $objResponse->EMPHIERCHY_DATE !=''?$objResponse->EMPHIERCHY_DATE:''); ?>" class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
           </div>
         </div>   
         ...
         <div class="row">
           ...
           <div class="col-lg-2 pl">
             <input <?php echo e($ActionStatus); ?> type="text" name="TEAM_NAME" id="TEAM_NAME" value="<?php echo e(isset($objResponse->EMPCODE_NAME) && $objResponse->EMPCODE_NAME !=''?$objResponse->EMPCODE_NAME:''); ?>" onclick="getTeamName(this.id,'<?php echo e(route('master',[$FormId,'getTeamMstName'])); ?>','Team Name Details')" class="form-control mandatory" readonly />
             <input type="hidden" name="TEAM_MSTID_REF" id="TEAM_MSTID_REF" value="<?php echo e(isset($objResponse->EMPID) && $objResponse->EMPID !=''?$objResponse->EMPID:''); ?>" class="form-control" autocomplete="off" />
           </div>

           <div class="col-lg-2 pl"><p>De-Activated</p></div>
            <div class="col-lg-1 pl pr">
            <input <?php echo e($ActionStatus); ?> type="checkbox"   name="DEACTIVATED"  id="deactive-checkbox_0" <?php echo e($objResponse->DEACTIVATED == 1 ? "checked" : ""); ?>  value='<?php echo e($objResponse->DEACTIVATED == 1 ? 1 : 0); ?>' tabindex="2"  >
            </div>
         </div>
         ...
         <div class="row">
          <div class="col-lg-2 pl"><p>Date of De-Activated</p></div>
             ...
             <div class="col-lg-2 pl"><p>Reporting To*</p></div>
              <div class="col-lg-2 pl">
                <input <?php echo e($ActionStatus); ?> type="text" name="REPORTING_TO" id="REPORTING_TO" value="<?php echo e(isset($objResponse->REPORTTOCODE_NAME) && $objResponse->REPORTTOCODE_NAME !=''?$objResponse->REPORTTOCODE_NAME:''); ?>" onclick="getTeamName(this.id,'<?php echo e(route('master',[$FormId,'getReportngTo'])); ?>','Reporting To Details')" class="form-control mandatory" readonly />
                <input type="hidden" name="REPORTING_TOID_REF" id="REPORTING_TOID_REF" value="<?php echo e(isset($objResponse->REPORTING_TOID_REF) && $objResponse->REPORTING_TOID_REF !=''?$objResponse->REPORTING_TOID_REF:''); ?>" class="form-control" autocomplete="off" />
              </div>
           </div>
                     ...
                     <tbody>
                      ...
                      <?php $__currentLoopData = $MAT; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                       ...
                       <tr  class="participantRow">
                         <td><input <?php echo e($ActionStatus); ?> class="form-control dynamic" type="text"  name="SRNo[]"       id ="SRNo_<?php echo e($row->SR_NO); ?>"              value="<?php echo e(isset($row->SR_NO) && $row->SR_NO !=''?$row->SR_NO:''); ?>"  autocomplete="off" readonly></td>
                         <td><input <?php echo e($ActionStatus); ?> class="form-control" type="text"          name="EMPCODE[]"    id ="EMPCODE_<?php echo e($key); ?>"                  value="<?php echo e(isset($row->EMPCODE_NAME) && $row->EMPCODE_NAME !=''?$row->EMPCODE_NAME:''); ?>"  onclick="getTeamName(this.id,'<?php echo e(route('master',[$FormId,'getEmpCode'])); ?>','Employee Code Details')" autocomplete="off" readonly></td>
                         <td hidden><input   class="form-control" type="hidden"                 name="EMPID_REF[]"  id ="HIDDEN_EMPCODE_<?php echo e($key); ?>"           value="<?php echo e(isset($row->EMPID) && $row->EMPID !=''?$row->EMPID:''); ?>"  autocomplete="off" readonly></td>
                         
                         
                         <td hidden><input class="form-control" type="hidden"          name="REPORTTOID_REF[]"  id ="HIDDEN_REPORTTO_<?php echo e($key); ?>"          value="<?php echo e(isset($row->REPORT_EMPID) && $row->REPORT_EMPID !=''?$row->REPORT_EMPID:''); ?>"  autocomplete="off" readonly></td>
                         <td><select <?php echo e($ActionStatus); ?>                                 name="EMPACTIVE[]" id="EMPACTIVE_<?php echo e($key); ?>" class="form-control"  autocomplete="off">
                          <option <?php echo e(isset($row->EMPACTIVE) && $row->EMPACTIVE == 'YES'?'selected="selected"':''); ?> value="YES">Yes</option>
                          <option <?php echo e(isset($row->EMPACTIVE) && $row->EMPACTIVE == 'NO'?'selected="selected"':''); ?> value="NO">No</option>
                         </select>
                       </td>
                         <td align="center">
                           <button <?php echo e($ActionStatus); ?> class="btn add" title="add" data-toggle="tooltip" id ="SRLNo_<?php echo e($row->SR_NO+1); ?>" onclick="SrNo(this.id)"><i class="fa fa-plus"></i></button>
...
function bindEmployeeEvents(ROW_ID){
 ...
 var reportid = $('#REPORTING_TOID_REF').val();
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\bsquareappfordemo.com\UNICORN\resources\views\masters\PreSales\EmployeeHierarchyMaster\mstfrm505edit.blade.php ENDPATH**/ ?>