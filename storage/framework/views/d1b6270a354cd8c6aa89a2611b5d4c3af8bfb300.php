...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="<?php echo e(route('transaction',[$FormId,'index'])); ?>" class="btn singlebt">Bank Reconcilation</a></div>
...
<form id="transaction_form" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >  
  <div class="container-fluid filter">
	  <div class="inner-form">
          ...
          <input  type="hidden" name="BANK_RECONCILEID" id="BANK_RECONCILEID" value="<?php echo e(isset($objResponse->BANK_RECONCILEID) && $objResponse->BANK_RECONCILEID !=''?$objResponse->BANK_RECONCILEID:''); ?>" class="form-control mandatory"  >
          <input <?php echo e($ActionStatus); ?> type="text" name="DOC_NO" id="DOC_NO" value="<?php echo e(isset($objResponse->BANK_RECONCILE_CODE) && $objResponse->BANK_RECONCILE_CODE !=''?$objResponse->BANK_RECONCILE_CODE:''); ?>" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
          <span class="text-danger" id="ERROR_DOC_NO"></span>
			  </div>
			
			  <div class="col-lg-1 pl"><p>Doc Date</p></div>
			  <div class="col-lg-2 pl">
				  <input <?php echo e($ActionStatus); ?> type="date" name="DOC_DT" id="DOC_DT"  value="<?php echo e(isset($objResponse->BANK_RECONCILE_DATE) && $objResponse->BANK_RECONCILE_DATE !=''?$objResponse->BANK_RECONCILE_DATE:''); ?>" class="form-control"  placeholder="dd/mm/yyyy" >
        </div>

        <div class="col-lg-1 pl"><p>Bank</p></div>
			  <div class="col-lg-2 pl">
          <input <?php echo e($ActionStatus); ?> type="text" name="TEXT_BID_REF" id="TEXT_BID_REF" value="<?php echo e(isset($objResponse->BANK_NAME) && $objResponse->BANK_NAME !=''?$objResponse->BANK_NAME:''); ?>" class="form-control" autocomplete="off" readonly onclick="showHideModal('show')" >
          <input type="hidden" name="BID_REF" id="BID_REF"  value="<?php echo e(isset($objResponse->CASH_BANK_ID) && $objResponse->CASH_BANK_ID !=''?$objResponse->CASH_BANK_ID:''); ?>" class="form-control" autocomplete="off" >
          <input type="hidden" name="GLID_REF" id="GLID_REF" value="<?php echo e(isset($objResponse->GLID_REF) && $objResponse->GLID_REF !=''?$objResponse->GLID_REF:''); ?>" class="form-control" autocomplete="off" >
			  </div>

        <div class="col-lg-1 pl"><p>Mode</p></div>
			  <div class="col-lg-2 pl">
          <select <?php echo e($ActionStatus); ?> name="MODE" id="MODE" class="form-control" autocomplete="off" onchange="getDataArray()" >
            <option <?php echo e(isset($objResponse->BANK_RECONCILE_MODE) && $objResponse->BANK_RECONCILE_MODE =='ALL'?'selected="selected"':''); ?> value="ALL">ALL</option>
            <option <?php echo e(isset($objResponse->BANK_RECONCILE_MODE) && $objResponse->BANK_RECONCILE_MODE =='NON RECONCILE'?'selected="selected"':''); ?> value="NON RECONCILE">NON RECONCILE</option>
          </select>
			  </div>
		  </div>
  ...
  <div class="modal-dialog modal-md" style="width:80%">
    ...
    <tr>
      ...
      <table id="BANK_TABLE2" class="display nowrap table  table-striped table-bordered" >
        ...
        <?php $__currentLoopData = $bankMaster; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$val): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <tr>
          <td style="width:10%;text-align:center;"> <input type="checkbox" name="SELECT_BID_REF[]" class="checkbox" value="<?php echo e($val-> BID); ?>" onChange="bindBank(this.value,'<?php echo e($val-> NAME); ?> - <?php echo e($val-> ACNO); ?>','<?php echo e($val-> GLID_REF); ?>')" ></td>
          <td style="width:10%;"><?php echo e($val-> BCODE); ?></td>
          <td style="width:20%;"><?php echo e($val-> NAME); ?></td>
          <td style="width:20%;"><?php echo e($val-> IFSC); ?></td>
          <td style="width:20%;"><?php echo e($val-> ACNO); ?></td>
          <td style="width:20%;"><?php echo e($val-> ACTYPE); ?></td>
        </tr>
...
function getDataArray(){
  ...
  var BANK_RECONCILEID  = "<?php echo e(isset($objResponse->BANK_RECONCILEID) && $objResponse->BANK_RECONCILEID !=''?$objResponse->BANK_RECONCILEID:''); ?>";
...
function balanceCalculation(){
  ...
  {
	  BALANCE_OF_BANK = (parseFloat(Math.abs(TOTAL_DF2)) + parseFloat(TOTAL_DEBIT_AMOUNT)) - parseFloat(TOTAL_CREDIT_AMOUNT); 
	  var TEXT_BANK_BL_CRDR     = BALANCE_OF_BANK >= 0 ?'DR':'CR';
  }  
	//alert(TOTAL_DF+''+BALANCE_OF_BANK);
  ...
  {
		TEXT_TOTAL_DF   = 	parseFloat(Math.abs(TOTAL_DF)) - parseFloat(BALANCE_OF_BANK);		
  }
  else if($("#TEXT_COMPANY_BL_CRDR").text() == 'DR' && $("#TEXT_BANK_BL_CRDR").text() == 'DR' && parseFloat(TOTAL_DF) < parseFloat(BALANCE_OF_BANK))
  {
		TEXT_TOTAL_DF   = parseFloat(BALANCE_OF_BANK) - parseFloat(Math.abs(TOTAL_DF));
  }
  else
  {
		TEXT_TOTAL_DF   = parseFloat(Math.abs(TOTAL_DF)) - parseFloat(BALANCE_OF_BANK);
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\bsquareappfordemo.com\UNICORN\resources\views\transactions\Accounts\BankReconcilation\trnfrm394edit.blade.php ENDPATH**/ ?>