...
<!-- <form id="frm_mst_se" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="<?php echo e(route('transaction',[$FormId,'index'])); ?>" class="btn singlebt">Sales Invoice Allocation</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view filter">     
      <form id="frm_mst_edit" onsubmit="return validateForm(actionType)" method="POST"  class="needs-validation"> 
        <?php echo csrf_field(); ?>
        <?php echo e(isset($HRD->SALINVALID) ? method_field('PUT') : ''); ?>

      <div class="inner-form">
      <div class="row">
        <div class="col-lg-2 pl"><p>Doc No*</p></div>
        <div class="col-lg-2 pl">
          <input <?php echo e($ActionStatus); ?> type="text" name="DOC_NO" id="DOC_NO" value="<?php echo e(isset($HRD->SALINVAL_NO) && $HRD->SALINVAL_NO !=''?$HRD->SALINVAL_NO:''); ?>" class="form-control mandatory"  autocomplete="off" readonly >
        </div>
        <div class="col-lg-2 pl"><p>Doc Date*</p></div>
          <div class="col-lg-2 pl">
            <input <?php echo e($ActionStatus); ?> type="date" name="DOC_DT" id="DOC_DT" onchange="checkPeriodClosing('<?php echo e($FormId); ?>',this.value,1)" value="<?php echo e(isset($HRD->SALINVAL_DATE) && $HRD->SALINVAL_DATE !=''?$HRD->SALINVAL_DATE:''); ?>" class="form-control mandatory"  autocomplete="off" >
        </div> 
          
        <div class="col-lg-2 pl"><p>From Date</p></div>
          <div class="col-lg-2 pl">
          <input <?php echo e($ActionStatus); ?> type="date" name="FROM_DATE" id="FROM_DATE" value="<?php echo e(isset($HRD->SALINVAL_FROM_DATE) && $HRD->SALINVAL_FROM_DATE !=''?$HRD->SALINVAL_FROM_DATE:''); ?>" class="form-control mandatory"  autocomplete="off" >
        </div>
        </div>
        <div class="row">
          <div class="col-lg-2 pl"><p>To Date*</p></div>
          <div class="col-lg-2 pl">
          <input <?php echo e($ActionStatus); ?> type="date" name="TO_DATE" id="TO_DATE" value="<?php echo e(isset($HRD->SALINVAL_TO_DATE) && $HRD->SALINVAL_TO_DATE !=''?$HRD->SALINVAL_TO_DATE:''); ?>" class="form-control mandatory"  autocomplete="off" >
          </div>
        </div> 
              ...
              <table id="example1" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                ...
                <tbody>
                  ...
                  <tr  class="participantRow">
                    <td><input <?php echo e($ActionStatus); ?> type="text" name="SALESINVOICENO[]"          id ="SALESINVOICENO_<?php echo e($key); ?>"  value="<?php echo e(isset($row->SINO) && $row->SINO !=''?$row->SINO:''); ?>" onclick="getSaleInvNo(this.id,this.value)"  class="form-control mandatory"  autocomplete="off" readonly/></td>
                    <td hidden><input type="hidden" name="SALESINVID_REF[]" id="SALESINVID_REF_<?php echo e($key); ?>"   value="<?php echo e(isset($row->SIID) && $row->SIID !=''?$row->SIID:''); ?>" class="form-control" autocomplete="off" /></td>
                    
                    <td><input <?php echo e($ActionStatus); ?> type="date" name="SALESINVOICEDATE"          id="SALESINVOICEDATE_<?php echo e($key); ?>" value="<?php echo e(isset($row->SIDT) && $row->SIDT !=''?$row->SIDT:''); ?>" class="form-control" readonly></td>                    
                    <td><input <?php echo e($ActionStatus); ?> type="text" name="CUSTOMERNAME[]"            id="CUSTOMERNAME_<?php echo e($key); ?>"     value="<?php echo e(isset($row->NAME) && $row->NAME !=''?$row->NAME:''); ?>" class="form-control" readonly></td>
                    <td><input <?php echo e($ActionStatus); ?> type="text" name="SALESINVOICEAMT[]"         id="SALESINVOICEAMT_<?php echo e($key); ?>"  value="<?php echo e(isset($row->SALINVOICE_AMT) && $row->SALINVOICE_AMT !=''?$row->SALINVOICE_AMT:''); ?>" class="form-control" readonly></td>
                    
                    <td><input <?php echo e($ActionStatus); ?> type="text" name="EMPLOYEENAME[]"            id="EMPLOYEENAME_<?php echo e($key); ?>"     value="<?php echo e(isset($row->EMPCODE) && $row->EMPCODE !=''?$row->EMPCODE:''); ?> <?php echo e(isset($row->FNAME) && $row->FNAME !=''?'- '.$row->FNAME:''); ?>" onclick="getEmplyName(this.id,this.value)" class="form-control mandatory" autocomplete="off" readonly></td>
                    <td hidden><input type="hidden" name="EMPID_REF[]"      id="EMPID_REF_<?php echo e($key); ?>"        value="<?php echo e(isset($row->EMPID) && $row->EMPID !=''?$row->EMPID:''); ?>" class="form-control" autocomplete="off" /></td>

                    <td><input <?php echo e($ActionStatus); ?> type="text" name="BIFURCATEAMOUNT[]"         id="BIFURCATEAMOUNT_<?php echo e($key); ?>"  value="<?php echo e(isset($row->BIFURCATED_AMT) && $row->BIFURCATED_AMT !=''?$row->BIFURCATED_AMT:''); ?>" class="form-control" onkeypress="return isNumberKey(this, event);"></td>
...
function submitData(type){
  var formResponseMst = $("#frm_mst_edit");
  if(formResponseMst.valid()){
    if(checkPeriodClosing('<?php echo e($FormId); ?>',$("#DOC_DT").val(),0) ==0){
      $("#YesBtn").hide();
      $("#NoBtn").hide();
      $("#OkBtn").hide();
      $("#OkBtn1").show();
...
$('#tbody_divpopp').html('Loading...');
  $.ajaxSetup({
    ...
    $.ajax({
        url:'<?php echo e(route("transaction",[$FormId,"getEmplyDetails"])); ?>',
        type:'POST',
        success:function(data) {
            $('#tbody_divpopp').html(data);
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\inetpub\bsquareappfordemo.com\UNICORN\resources\views\transactions\sales\SalesInvoiceAllocation\trnfrm499edit.blade.php ENDPATH**/ ?>