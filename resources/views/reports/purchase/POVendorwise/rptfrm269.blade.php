...
<!-- <form id="frm_rpt_posvw" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-4">
                <a href="{{route('report',[269,'index'])}}" class="btn singlebt">Purchase Order Summary Vendor Wise</a>
                </div><!--col-2-->
                        ...
                        <div class="row">                            
                            ...
                            <div class="col-lg-2 pl">
                                <select name="VENDORGRP[]" id="VENDORGRP" class="form-control selectpicker" multiple data-live-search="true"  >
                                    @foreach($objVendorGroup as $index=>$Row)
                                        <option value="{{$Row->VGID}}" selected>{{$Row->VGCODE}}-{{$Row->DESCRIPTIONS}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-lg-2 pl">
                                <select name="VENDOR[]" id="VENDOR" class="form-control selectpicker" multiple data-live-search="true"  >
                                    @foreach($objVendor as $iindex=>$iRow)
                                        <option value="{{$iRow->VID}}" selected>{{$iRow->VCODE}}-{{$iRow->NAME}}</option>
                                    @endforeach
                                </select>
                            </div>
...
//                 });
//                 $.ajax({
//                     url:'{{route("report",[269,"GetPurchaseOrder"])}}',
//                     type:'POST',
...
$('#btnView').on('click', function() {
    var From_Date       = $('#From_Date').val();
        ...
        else if(VENDOR  == '')
        {
            ...
            var consultURL = '{{route("report",[269,"ViewReport",":rcdId"]) }}';
