...
<!-- <form id="frm_rpt_pop" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('report',[509,'index'])}}" class="btn singlebt">Bill of Material Amendment Print</a>
                </div><!--col-2-->
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3 pl">
                                <select name="IPO_NO" id="IPO_NO" class="form-control mandatory">
                                    ...
                                    @foreach($ObjBOM as $index=>$Row)
                                        <option value="{{$Row->BOMID_REF}}" >{{$Row->BOM_NO}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
$(document).ready(function(e) {
   ...
   $('#IPO_NO').on('change', function() {
	   if($(this).val() !== '')
	   {
				var POID = $(this).val();
                  ...
                  $.ajax({
                      url:'{{route("report",[509,"BOMAmendment"])}}',
                      type:'POST',
                      data:{'BOMID_REF':POID},
   ...
   $('#btnPdf').on('click', function() {
    ...
    var consultURL = '{{route("report",[509,"ViewReport",":rcdId"]) }}';
...
$('#btnView').on('click', function() {
        ...
        else
        {
            ...
            $(".buttonload").show();
            $.ajax({
                url:'{{route("report",[509,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $("#btnView").show();               
