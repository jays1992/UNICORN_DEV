...
<!-- <form id="frm_rpt_sop" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('report',[297,'index'])}}" class="btn singlebt">Sales Order Print</a>
                </div><!--col-2-->
                            ...
                            <div class="col-lg-3 pl">
                                <select name="SONO" id="SONO" class="form-control mandatory">
                                    ...
                                    @foreach($objSono as $index=>$Row)
                                        <option value="{{$Row->SOID}}" >{{$Row->SONO}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
$(document).ready(function(e) {
   ...
   $('#btnPdf').on('click', function() {
    ...
    var consultURL = '{{route("report",[297,"ViewReport",":rcdId"]) }}';
...
$('#btnView').on('click', function() {
        ...
        else
        {
            ...
            $(".buttonload").show();
            $.ajax({
                url:'{{route("report",[297,"ViewReport"])}}',
                type:'POST',
                data:formData,
