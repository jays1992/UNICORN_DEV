...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MISPID}}" value="{{$val->MISPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MISP_DT) && $val->MISP_DT !='' && $val->MISP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MISP_DT)):''}}" ></td>
                <td>{{isset($val->MISP_NO) && $val->MISP_NO !=''?$val->MISP_NO:''}}</td>
                <td>{{isset($val->MISP_DT) && $val->MISP_DT !='' && $val->MISP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MISP_DT)):''}}</td>
