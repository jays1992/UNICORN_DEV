...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MRSPID}}" value="{{$val->MRSPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MRSP_DT) && $val->MRSP_DT !='' && $val->MRSP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MRSP_DT)):''}}" ></td>
                <td>{{isset($val->MRSP_NO) && $val->MRSP_NO !=''?$val->MRSP_NO:''}}</td>
                <td>{{isset($val->MRSP_DT) && $val->MRSP_DT !='' && $val->MRSP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MRSP_DT)):''}}</td>
