...
<div class="container-fluid topnav">
        <div class="row">
            <div class="col-lg-2">
            <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Additional Material Requisition (AMR)</a>
            </div>
...
<form id="transaction_form"  method="POST">   
    @csrf
    {{isset($objResponse->MRSPID[0]) ? method_field('PUT') : '' }}
	...
	<div class="inner-form">
		...
		<div class="row">
          ...
          <input {{$ActionStatus}} type="text" name="MRS_NO" id="MRS_NO" value="{{isset($objResponse->MRSP_NO) && $objResponse->MRSP_NO !=''?$objResponse->MRSP_NO:''}}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase" >
      </div>
			
			<div class="col-lg-1 pl"><p>AMR Date</p></div>
			<div class="col-lg-2 pl">
			    <input {{$ActionStatus}} type="date" name="MRS_DT" id="MRS_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{isset($objResponse->MRSP_DT) && $objResponse->MRSP_DT !=''?$objResponse->MRSP_DT:''}}" class="form-control mandatory"  placeholder="dd/mm/yyyy" >
      </div>

      <div class="col-lg-1 pl"><p>PRO No</p></div>
			<div class="col-lg-2 pl">
          <input {{$ActionStatus}} type="text" name="PRO_NO_popup" id="txtPRO_NO_popup" value="{{isset($objProductionOrderNo->PRO_NO) && $objProductionOrderNo->PRO_NO !=''?$objProductionOrderNo->PRO_NO:''}}" class="form-control mandatory"  autocomplete="off"  readonly/>
          <input type="hidden" name="PRO_NO" id="PRO_NO" class="form-control" autocomplete="off" value="{{isset($objResponse->PROID_REF) && $objResponse->PROID_REF !=''?$objResponse->PROID_REF:''}}" />
			</div>
		</div>
