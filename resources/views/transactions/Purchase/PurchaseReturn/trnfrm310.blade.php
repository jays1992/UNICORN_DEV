...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            <tr>
                <td><input type="checkbox" id="chkId{{$val->PRRID}}" value="{{$val->PRRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->PRR_DT) && $val->PRR_DT !='' && $val->PRR_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->PRR_DT)):''}}" ></td>
                <td>{{isset($val->PRR_NO) && $val->PRR_NO !=''?$val->PRR_NO:''}}</td>
                ...
                <td>{{isset($val->VCL_NO) && $val->VCL_NO !=''?$val->VCL_NO:''}}</td>
                <td>{{isset($val->DRIVER_NAME) && $val->DRIVER_NAME !=''?$val->DRIVER_NAME:''}}</td>
