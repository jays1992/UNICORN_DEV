...
@section('content')    
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->VQCID}}" value="{{$val->VQCID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->VQC_DT) && $val->VQC_DT !='' && $val->VQC_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->VQC_DT)):''}}" ></td>
              <td>{{isset($val->VQC_NO) && $val->VQC_NO !=''?$val->VQC_NO:''}}</td>
              <td>{{isset($val->VQC_DT) && $val->VQC_DT !='' && $val->VQC_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->VQC_DT)):''}}</td>
