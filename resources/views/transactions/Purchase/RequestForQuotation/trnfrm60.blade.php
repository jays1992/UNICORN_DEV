...
@section('content')    
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->RFQID}}" value="{{$val->RFQID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->RFQ_DT) && $val->RFQ_DT !='' && $val->RFQ_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->RFQ_DT)):''}}" ></td>
              <td>{{isset($val->RFQ_NO) && $val->RFQ_NO !=''?$val->RFQ_NO:''}}</td>
              <td>{{isset($val->RFQ_DT) && $val->RFQ_DT !='' && $val->RFQ_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->RFQ_DT)):''}}</td>
