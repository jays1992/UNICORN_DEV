...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Commercial Invoice</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->CINV_ID}}" value="{{$val->CINV_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->CINV_DT) && $val->CINV_DT !='' && $val->CINV_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->CINV_DT)):''}}" ></td>
              <td>{{isset($val->CINV_NO) && $val->CINV_NO !=''?$val->CINV_NO:''}}</td>
              <td>{{isset($val->CINV_DT) && $val->CINV_DT !='' && $val->CINV_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->CINV_DT)):''}}</td>
              <td>{{isset($val->CINV_STATUS) && $val->CINV_STATUS !=''?$val->CINV_STATUS:''}}</td>
