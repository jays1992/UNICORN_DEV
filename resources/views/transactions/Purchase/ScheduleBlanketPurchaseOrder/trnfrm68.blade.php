...
@section('content')    
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                  <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Schedule against Blanket<br/> Purchase Order</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->SBPID}}" value="{{$val->SBPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->SBP_DT) && $val->SBP_DT !='' && $val->SBP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->SBP_DT)):''}}" ></td>
              <td>{{isset($val->SBP_NO) && $val->SBP_NO !=''?$val->SBP_NO:''}}</td>
              <td>{{isset($val->SBP_DT) && $val->SBP_DT !='' && $val->SBP_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->SBP_DT)):''}}</td>
              <td>{{isset($val->SLNAME) && $val->SLNAME !=''?$val->SLNAME:''}}</td>
              <td>{{isset($val->TITLE) && $val->TITLE !=''?$val->TITLE:''}}</td>
