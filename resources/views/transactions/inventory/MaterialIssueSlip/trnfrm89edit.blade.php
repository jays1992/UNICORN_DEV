...
<form id="trn_frm_mis"  method="POST">   
    @csrf
    {{isset($objResponse->MISID[0]) ? method_field('PUT') : '' }}
	...
	<div class="inner-form">
		...
		<div class="row">
          ...
          <input {{$ActionStatus}} type="text" name="MIS_NO" id="MIS_NO" value="{{isset($objResponse->MIS_NO) && $objResponse->MIS_NO !=''?$objResponse->MIS_NO:''}}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
      </div>
			
			<div class="col-lg-1 pl"><p>MIS Date <strong>*</strong></p></div>
			<div class="col-lg-2 pl">
			    <input {{$ActionStatus}} type="date" name="MIS_DT" id="MIS_DT" onchange="checkPeriodClosing('{{$FormId}}',this.value,1)" value="{{isset($objResponse->MIS_DT) && $objResponse->MIS_DT !=''?$objResponse->MIS_DT:''}}" class="form-control mandatory" >
      </div>
		...
		<div class="row">
			...
			<div class="tab-content">
              ...
              @foreach($objMAT as $key => $row)
                  ...
                  <td><input {{$ActionStatus}} style="width:100px;" type="text" name="txtRGP_popup_{{$key}}" id="txtRGP_popup_{{$key}}" value="{{ $row->MRS_NO }}" class="form-control"  autocomplete="off"  readonly style="width:100px;" /></td>
                  <td hidden><input type="hidden" name="RGP_NO_{{$key}}" id="RGP_NO_{{$key}}" value="{{ $row->MRSID_REF }}"  class="form-control" autocomplete="off" /></td>
                  ...
                  <td><input {{$ActionStatus}} type="text" name={{"SE_QTY_".$key}} id={{"SE_QTY_".$key}} class="form-control three-digits" readonly onkeypress="return isNumberDecimalKey(event,this)" maxlength="13" value="{{ $row->MRS_QTY_BL}}" autocomplete="off"  /></td>
