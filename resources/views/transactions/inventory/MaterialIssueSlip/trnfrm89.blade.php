...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MISID}}" value="{{$val->MISID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MIS_DT) && $val->MIS_DT !='' && $val->MIS_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MIS_DT)):''}}" ></td>
                <td>{{isset($val->MIS_NO) && $val->MIS_NO !=''?$val->MIS_NO:''}}</td>
                <td>{{isset($val->MIS_DT) && $val->MIS_DT !='' && $val->MIS_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MIS_DT)):''}}</td>
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var consultURL = '{{route("transaction",[89,"ViewReport",":rcdId"]) }}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[89,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
