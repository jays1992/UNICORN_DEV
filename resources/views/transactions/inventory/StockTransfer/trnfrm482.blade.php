...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('transaction',[$FormId,'index'])}}" class="btn singlebt">Stock Transfer</a>
                </div>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp            
            <tr>
                <td><input type="checkbox" id="chkId{{$val->STID}}" value="{{$val->STID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->STDT) && $val->STDT !='' && $val->STDT !='1900-01-01' ? date('d-m-Y',strtotime($val->STDT)):''}}" ></td>
                <td>{{isset($val->STNO) && $val->STNO !=''?$val->STNO:''}}</td>
                <td>{{isset($val->STDT) && $val->STDT !='' && $val->STDT !='1900-01-01' ? date('d-m-Y',strtotime($val->STDT)):''}}</td>
                <td>{{isset($val->NATURE) && $val->NATURE !=''?$val->NATURE:''}}</td>
                <td>{{isset($val->CUST_ADDRESS) && $val->CUST_ADDRESS !=''?$val->CUST_ADDRESS:''}}</td>
                <td>{{isset($val->GSTIN_NO) && $val->GSTIN_NO !=''?$val->GSTIN_NO:''}}</td>
