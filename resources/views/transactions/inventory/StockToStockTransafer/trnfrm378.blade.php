    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            <tr>
                ...
                <th>Doc No</th>
                <th>Date</th>
                <th {{$AlpsStatus['hidden']}}>{{isset($TabSetting->FIELD8) && $TabSetting->FIELD8 !=''?'OUT '.$TabSetting->FIELD8:'Add. Info Part No'}}</th>
                <th {{$AlpsStatus['hidden']}}>{{isset($TabSetting->FIELD8) && $TabSetting->FIELD8 !=''?'IN '.$TabSetting->FIELD8:'Add. Info Part No'}}</th>
                <th>Type</th> 
                <th>Qty</th>
                <th>Created Date</th>    
                <th>Created By</th>           
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->ST_STID}}" value="{{$val->ST_STID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->ST_ST_DOCDT) && $val->ST_ST_DOCDT !='' && $val->ST_ST_DOCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->ST_ST_DOCDT)):''}}" ></td>
                <td>{{isset($val->ST_ST_DOCNO) && $val->ST_ST_DOCNO !=''?$val->ST_ST_DOCNO:''}}</td>
                <td>{{isset($val->ST_ST_DOCDT) && $val->ST_ST_DOCDT !='' && $val->ST_ST_DOCDT !='1900-01-01' ? date('d-m-Y',strtotime($val->ST_ST_DOCDT)):''}}</td>
                <td {{$AlpsStatus['hidden']}}>{{isset($val->OUT_ALPS_PART_NO) && $val->OUT_ALPS_PART_NO !=''?$val->OUT_ALPS_PART_NO:''}}</td>
                <td {{$AlpsStatus['hidden']}}>{{isset($val->IN_ALPS_PART_NO) && $val->IN_ALPS_PART_NO !=''?$val->IN_ALPS_PART_NO:''}}</td>
                <td>{{isset($val->ST_ST_TYPE) && $val->ST_ST_TYPE !=''?$val->ST_ST_TYPE:''}}</td>
                <td>{{isset($val->QTY) && $val->QTY !=''?$val->QTY:''}}</td>
