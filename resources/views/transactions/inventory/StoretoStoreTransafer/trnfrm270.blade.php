...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->STTOSTID}}" value="{{$val->STTOSTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->STTOST_DOCDT) && $val->STTOST_DOCDT !=''?$val->STTOST_DOCDT:''}}" ></td>
                <td style="text-transform:uppercase">{{isset($val->STTOST_DOCNO) && $val->STTOST_DOCNO !=''?$val->STTOST_DOCNO:''}}</td>
                <td>{{isset($val->STTOST_DOCDT) && $val->STTOST_DOCDT !=''?$val->STTOST_DOCDT:''}}</td>
        ...
        <form id="transactionForm270Print" action="{{ route('transactiongetlist',[270,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[270,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("transaction",[270,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[270,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[270,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[270,"cancel"])}}',
                            type:'POST',
