...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MRSID}}" value="{{$val->MRSID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->MRS_DT) && $val->MRS_DT !='' && $val->MRS_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MRS_DT)):''}}" ></td>
                <td>{{isset($val->MRS_NO) && $val->MRS_NO !=''?$val->MRS_NO:''}}</td>
                <td>{{isset($val->MRS_DT) && $val->MRS_DT !='' && $val->MRS_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->MRS_DT)):''}}</td>
                <td>{{isset($val->MRS_TYPE) && $val->MRS_TYPE !=''?$val->MRS_TYPE:''}}</td>
