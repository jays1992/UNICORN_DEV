...
<form id="transaction_form" onsubmit="return validateForm()"  method="POST" class="needs-validation"  >  
  ...
  <div class="container-fluid filter">
      ...
      <div class="col-lg-6">
        <div class="sub-box">
          <input type="checkbox" name="selectAll[]" value="{{ $row_data->BRC_BRCID }}" class="checkbox" >
          <div class="row sub-box-contant">
            ...
            <div class="col-lg-12" style="margin-top:20px;"><label>Serial No:</label> {{ $row_data->SERIALNUMBER }}</div>
            <div class="col-lg-12"><label>Item Code:</label> {{ $row_data->ICODE }}</div>
            <div class="col-lg-12"><label>Item Name:</label> {{ $row_data->ITEM_NAME }}</div>
            <div class="col-lg-12"><label>Item Group:</label> {{ $row_data->GROUPNAME }}</div>
...
window.fnDownloadData = function (){
  ...
  var viewURL = '{{route("transaction",[$FormId,"barcodepdf",":rcdId"]) }}';
