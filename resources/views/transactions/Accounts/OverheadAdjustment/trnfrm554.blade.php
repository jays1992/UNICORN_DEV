...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->OHID}}" value="{{$val->OHID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->OHDT) && $val->OHDT !=''?$val->OHDT:''}}" ></td>
                <td>{{isset($val->OHNO) && $val->OHNO !=''?$val->OHNO:''}}</td>
                <td>{{isset($val->OHDT) && $val->OHDT !=''?$val->OHDT:''}}</td>               
