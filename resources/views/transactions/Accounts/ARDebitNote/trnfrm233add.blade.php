...
@section('content')
	...
	<div class="inner-form">
        ...
        <div class="col-lg-2 pl">
            <input type="text" name="AR_DOC_NO" id="AR_DOC_NO" value="{{$docarray['DOC_NO']}}" {{$docarray['READONLY']}} class="form-control" maxlength="{{$docarray['MAXLENGTH']}}" autocomplete="off" style="text-transform:uppercase" >
            <script>docMissing(@json($docarray['FY_FLAG']));</script>
        </div>
        <div class="col-lg-2 pl"><p>Debit/Credit Date</p></div>
        <div class="col-lg-2 pl">
                  <input type="date" name="AR_DOC_DT" id="AR_DOC_DT" value="{{ old('AR_DOC_DT') }}" onchange='checkPeriodClosing(233,this.value,1),getDocNoByEvent("AR_DOC_NO",this,@json($doc_req))'  class="form-control mandatory"  placeholder="dd/mm/yyyy" />
        </div>
...
function getBalanceGrid(bankid,fieldid){
  ...
  var TaxStatus = $.ajax({type: 'POST',
  url:'{{route("transaction",[233,"getBalanceGrid"])}}',
  async: false,
  dataType: 'json',
  data: {id:bankid},
  done: function(response) {return response;}}).responseText;
