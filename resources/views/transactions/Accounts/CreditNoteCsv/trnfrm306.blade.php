...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp            
            <tr>
                <td><input type="checkbox" id="chkId{{$val->CSVID}}" value="{{$val->CSVID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->CSV_DT) && $val->CSV_DT !='' && $val->CSV_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->CSV_DT)):''}}" ></td>
                <td>{{isset($val->CSV_NO) && $val->CSV_NO !=''?$val->CSV_NO:''}}</td>
                <td>{{isset($val->CSV_DT) && $val->CSV_DT !='' && $val->CSV_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->CSV_DT)):''}}</td>
                <td>{{isset($val->SLNAME) && $val->SLNAME !=''?$val->SLNAME:''}}</td>
                <td>{{isset($val->REASON_CR_NOTE) && $val->REASON_CR_NOTE !=''?$val->REASON_CR_NOTE:''}}</td>
