...
@section('content')
	...
	<div class="inner-form">
        ...
        <div class="col-lg-2 pl">
              <input type="hidden" name="JVID" id="JVID" value="{{ isset($objJV->JVID)?$objJV->JVID:'' }}" />
              <input {{$ActionStatus}} type="text" name="JV_NO" id="JV_NO" value="{{ isset($objJV->JV_NO)?$objJV->JV_NO:'' }}" class="form-control mandatory"  autocomplete="off" readonly style="text-transform:uppercase"  >
         </div>
        <div class="col-lg-1 pl"><p>JV Date</p></div>
        <div class="col-lg-2 pl">
                  <input {{$ActionStatus}} type="date" name="JV_DT" id="JV_DT" value="{{ isset($objJV->JV_DT)?$objJV->JV_DT:'' }}" onchange="checkPeriodClosing(169,this.value,1)" class="form-control mandatory"  placeholder="dd/mm/yyyy" />
        </div>
...
$.ajax({
    url:'{{ route("transaction",[169,"Approve"])}}',
    type:'POST',
    data:formData,
    success:function(data) {
      $("#btnApprove").show();  
