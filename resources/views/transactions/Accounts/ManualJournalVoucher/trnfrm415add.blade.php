...
@section('content')
	...
	<div class="inner-form">
        ...
        <div class="col-lg-2 pl">
              <input type="text" name="MJV_NO" id="MJV_NO" value="{{$docarray['DOC_NO']}}" {{$docarray['READONLY']}} class="form-control" maxlength="{{$docarray['MAXLENGTH']}}" autocomplete="off" style="text-transform:uppercase" >
              <script>docMissing(@json($docarray['FY_FLAG']));</script>
        </div>
        <div class="col-lg-1 pl"><p>MJV Date </p></div>
        <div class="col-lg-2 pl">
                  <input type="date" name="MJV_DT" id="MJV_DT" value="{{ old('MJV_DT') }}" onchange='checkPeriodClosing(415,this.value,1),getDocNoByEvent("MJV_NO",this,@json($doc_req))' class="form-control mandatory"  placeholder="dd/mm/yyyy" <?php if(isset($DOC_TYPE) && $DOC_TYPE !=''){?> onchange='getDocNoByEvent("MJV_NO",this,<?php echo json_encode($doc_req);?>)' <?php } ?> />
        </div>
