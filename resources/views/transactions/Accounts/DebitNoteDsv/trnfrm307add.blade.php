    ...
    <div class="container-fluid filter">
      <div class="inner-form">
        ...
        <div class="row">
          ...
          <input type="text" name="DSV_NO" id="DSV_NO" value="{{$docarray['DOC_NO']}}" {{$docarray['READONLY']}} class="form-control" maxlength="{{$docarray['MAXLENGTH']}}" autocomplete="off" style="text-transform:uppercase" >
          <script>docMissing(@json($docarray['FY_FLAG']));</script>
          </div>
                              
          <div class="col-lg-2 pl"><p>DSV Date*</p></div>
          <div class="col-lg-2 pl">
              <input type="date" name="DSV_DT" id="DSV_DT" value="{{ old('DSV_DT') }}" onchange='checkPeriodClosing("{{$FormId}}",this.value,1),getDocNoByEvent("DSV_NO",this,@json($doc_req))' class="form-control mandatory" autocomplete="off" placeholder="dd/mm/yyyy" >
          </div>
...
function getTransactionType(value){
  ...
  $.ajaxSetup({
      ...
      $.ajax({
        url:'{{route("transaction",[$FormId,"getDSVNo"])}}',
        type:'POST',
        data:{dataval:value},
        success:function(data) {
            $("#DSV_NO").val(data);                
