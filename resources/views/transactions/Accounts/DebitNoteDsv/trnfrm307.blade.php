...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            <tr>
                <td><input type="checkbox" id="chkId{{$val->DSVID}}" value="{{$val->DSVID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->DSV_DT) && $val->DSV_DT !='' && $val->DSV_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->DSV_DT)):''}}" ></td>
                <td>{{isset($val->DSV_NO) && $val->DSV_NO !=''?$val->DSV_NO:''}}</td>
                <td>{{isset($val->DSV_DT) && $val->DSV_DT !='' && $val->DSV_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->DSV_DT)):''}}</td>
                <td>{{isset($val->SLNAME) && $val->SLNAME !=''?$val->SLNAME:''}}</td>
                <td>{{isset($val->REASON_DR_NOTE) && $val->REASON_DR_NOTE !=''?$val->REASON_DR_NOTE:''}}</td>
