...
<!-- <form id="frm_mst_se" onsubmit="return validateForm()"  method="POST"  >     -->
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                {{-- <button class="btn topnavbt" onclick="submitData('fnApproveData')"  id="btnApprove" {{ (isset($objRights->APPROVAL1) || isset($objRights->APPROVAL2) || isset($objRights->APPROVAL3) || isset($objRights->APPROVAL4) || isset($objRights->APPROVAL5)) &&  ($objRights->APPROVAL1||$objRights->APPROVAL2||$objRights->APPROVAL3||$objRights->APPROVAL4||$objRights->APPROVAL5) == 1 ? '' : 'disabled'}}><i class="fa fa-thumbs-o-up"></i> Approved</button> --}}
    ...
    <div class="container-fluid purchase-order-view filter">     
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl">
          ...
          <input {{$ActionStatus}} type="text"  id="CUSTOMERPROSPECT_NAME" onclick="getCustProspect()"  value="{{isset($objCustProspt->CCODE) && $objCustProspt->CCODE !=''?$objCustProspt->CCODE:''}} {{isset($objCustProspt->CUSTNAME) && $objCustProspt->CUSTNAME !=''?'- '.$objCustProspt->CUSTNAME:''}}{{isset($objCustProspt->PCODE) && $objCustProspt->PCODE !=''?$objCustProspt->PCODE:''}} {{isset($objCustProspt->PROSNAME) && $objCustProspt->PROSNAME !=''?'- '.$objCustProspt->PROSNAME:''}}" class="form-control mandatory"  autocomplete="off" readonly/>
          <input {{$ActionStatus}} type="hidden" name="CUSTOMER_PROSPECT" id="CUSTOMER_PROSPECT" value="{{isset($objCustProspt->CID) && $objCustProspt->CID !=''?$objCustProspt->CID:''}}{{isset($objCustProspt->PID) && $objCustProspt->PID !=''?$objCustProspt->PID:''}}" class="form-control" autocomplete="off" />
        </div>
        ...
        <div class="row">
            ...
            <input {{$ActionStatus}} type="hidden" name="FNAME" id="FNAME" value="{{isset($objResponse->FIRST_NAME) && $objResponse->FIRST_NAME !=''?$objResponse->FIRST_NAME:''}}" class="form-control mandatory" autocomplete="off">                            
            <input {{$ActionStatus}} type="hidden" name="LNAME" id="LNAME" value="{{isset($objResponse->LAST_NAME) && $objResponse->LAST_NAME !=''?$objResponse->LAST_NAME:''}}" class="form-control mandatory" autocomplete="off">                            
            ...
            <div class="col-lg-2 pl">
              <textarea {{$ActionStatus}} name="ADDRESS" id="ADDRESS" style="width: 192px;" class="form-control mandatory" readonly>{{isset($objResponse->ADDRESS) && $objResponse->ADDRESS !=''?$objResponse->ADDRESS:''}}</textarea>
            </div>
          </div>
          ...
          <div class="col-lg-2 pl"><p>City*</p></div>
            <div class="col-lg-2 pl">
              <input {{$ActionStatus}} type="text" name="CITYID_REF_POPUP" id="CITYID_REF_POPUP" value="{{isset($objResponse->CITYNAME) && $objResponse->CITYNAME !=''?$objResponse->CITYNAME:''}}" class="form-control mandatory" readonly tabindex="1" readonly/>
        ...
        <div class="row">
            ...
            <div class="col-lg-2 pl">
              <input {{$ActionStatus}} type="text" name="PINCODE" id="PINCODE" value="{{isset($objResponse->PINCODE) && $objResponse->PINCODE !=''?$objResponse->PINCODE:''}}" onkeypress="return onlyNumberKey(event)" maxlength="6" class="form-control mandatory" autocomplete="off" readonly>                             
            </div>
          
          <div class="col-lg-2 pl"><p>Lead Owner*</p></div>
          <div class="col-lg-2 pl">
            <input {{$ActionStatus}} type="text" name="LOWNER" id="LOWNER" value="{{isset($objResponse->EMPCODE) && $objResponse->EMPCODE !=''?$objResponse->EMPCODE:''}} - {{$objResponse->FNAME}} {{$objResponse->MNAME}} {{$objResponse->LNAME}}" class="form-control mandatory"  autocomplete="off" readonly/>
      ...
      <div class="row">
        ...
        <div class="col-lg-2 pl">
          <input {{$ActionStatus}} type="text" name="ASSIGTO" id="ASSIGTO" value="{{isset($objTlead->ASSGNTOCODE) && $objTlead->ASSGNTOCODE !=''?$objTlead->ASSGNTOCODE:''}} - {{$objTlead->ASSGNTOFNAME}} {{$objTlead->ASSGNTOMNAME}} {{$objTlead->ASSGNTOLNAME}}" class="form-control mandatory"  autocomplete="off" readonly/>
          <input {{$ActionStatus}} type="hidden" name="ASSIGTOID_REF" id="ASSIGTOID_REF" value="{{isset($objTlead->ASSIGNTO_REF) && $objTlead->ASSIGNTO_REF !=''?$objTlead->ASSIGNTO_REF:''}}" class="form-control" autocomplete="off" />
        </div>
      </div>
                    ...
                    <tbody>
                      ...
                      <tr  class="participantRow">
                        ...
                        <td>
                          ...
                          <option {{isset($row->PRIORITYID_REF) && $row->PRIORITYID_REF == $val->PAYPERIODID ?'selected="selected"':''}} value="{{ $val->PAYPERIODID }}">{{isset($val->PAY_PERIOD_DESC) && $val->PAY_PERIOD_DESC !=''?$val->PAY_PERIOD_DESC:''}}</option>  
                          @endforeach
                          </select> 
                        </td>
            ...
            <div class="table-responsive table-wrapper-scroll-y my-custom-scrollbar" style="height:280px;margin-top:10px;" >
              <table id="example5" class="display nowrap table table-striped table-bordered itemlist" width="100%" style="height:auto !important;">
                ...
                <tbody>
                  ...
                  <tr  class="participantRow">
                    <td><input {{$ActionStatus}} type="text" name="PRODUCTNAME[]" id ={{"PRODUCTNAME_".$key}} onclick="getProductName(this.id)"   value="{{isset($row->ICODE) && $row->ICODE !=''?$row->ICODE:''}}" class="form-control mandatory"  autocomplete="off" readonly/></td>
