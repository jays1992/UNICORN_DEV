...
@section('content')
    ...
    <div class="container-fluid purchase-order-view filter">     
        <form id="frm_calculation_attachment" method="post" enctype="multipart/form-data" action='{{route("transactionmodify",[$FormId,"docuploads"])}}' > 
          @CSRF
        ...
        <input type="hidden" name="VTID_REF" class="form-control" value='{{$objMstVoucherType[0]->VTID}}' />
			...
			<div class="col-lg-2 pl">
         <label> {{$objRes->LEAD_NO}} </label>
        <input type="hidden" name="ATTACH_DOCNO" id="ATTACH_DOCNO" value="{{$objRes->LEADID_REF}}" class="form-control" maxlength="50" >
			</div>
			
			<div class="col-lg-1 pl"><p>Document Date</p></div>
			<div class="col-lg-2 pl">
      <label> {{ date("d/m/Y", strtotime($objRes->INDATE) )}} </label> 
        <input type="hidden" name="ATTACH_DOCDT" id="ATTACH_DOCDT" value='{{date("Y-m-d", strtotime($objRes->INDATE))}}'   />
 ...
 var formCalculationMst = $( "#frm_calculation_attachment" );
    ...
    $("#OkBtn").click(function(){
       ...
       $("#"+$(this).data('focusname')).focus();
        ...
        //window.location.href = '{{route("transaction",[4,"index"]) }}';
