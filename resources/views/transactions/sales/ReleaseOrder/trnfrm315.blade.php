...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->ROID}}" value="{{$val->ROID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->RODT) && $val->RODT !='' && $val->RODT !='1900-01-01' ? date('d-m-Y',strtotime($val->RODT)):''}}" ></td>
              <td>{{isset($val->RONO) && $val->RONO !=''?$val->RONO:''}}</td>
              <td>{{isset($val->RODT) && $val->RODT !='' && $val->RODT !='1900-01-01' ? date('d-m-Y',strtotime($val->RODT)):''}}</td>
              <td>{{isset($val->SLNAME) && $val->SLNAME !=''?$val->SLNAME:''}}</td>
              <td>{{isset($val->OVFRM_DT) && $val->OVFRM_DT !='' && $val->OVFRM_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->OVFRM_DT)):''}}</td>
              <td>{{isset($val->OVTOM_DT) && $val->OVTOM_DT !='' && $val->OVTOM_DT !='1900-01-01' ? date('d-m-Y',strtotime($val->OVTOM_DT)):''}}</td>
          ...
          var ROIDsData = getSeletectedCBox();
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  var viewURL = '{{route("transaction",[315,"view",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[315,"attachment",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[315,"edit",":rcdId"]) }}';
          ...
          if(seletedRecord==0){
              ...
              highlighFocusBtn('activeOk1');
                ...
                if(is_approve==1){
                  ...
                  var editURL = '{{route("transaction",[315,"amendment",":rcdId"]) }}';
                      editURL = editURL.replace(":rcdId",recordId);
...
var recordId = ROIDsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[315,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[315,"cancel"])}}',
                            type:'POST',
  ...
  var soIdsData = getSeletectedCBox();
    var seletedRecord = soIdsData.length;
      ...
      }else if(seletedRecord==1)
      {
            ...
            var consultURL = '{{route("transaction",[315,"ViewReport",":rcdId"])}}';
...
$('#btnPrint').on('click', function() {
      ...
      }else if(seletedRecord==1)
      {
            ...
            $.ajax({
                url:'{{route("transaction",[315,"ViewReport"])}}',
                type:'POST',
                data:formData,
                success:function(data) {
                    $('#ReportView').show();
