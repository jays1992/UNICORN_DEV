  ...
  <div class="row">
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->DCID}}" value="{{$val->DCID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->DC_DATE) && $val->DC_DATE !='' && $val->DC_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DC_DATE)):''}}" ></td>
                <td>{{isset($val->DC_NO) && $val->DC_NO !=''?$val->DC_NO:''}}</td>
                <td>{{isset($val->DC_DATE) && $val->DC_DATE !='' && $val->DC_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DC_DATE)):''}}</td>
                <td>{{isset($val->DC_TYPE) && $val->DC_TYPE !=''?$val->DC_TYPE:''}}</td>
