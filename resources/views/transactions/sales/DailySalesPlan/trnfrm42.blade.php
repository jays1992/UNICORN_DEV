...
@section('content')    
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->DSPID}}" value="{{$val->DSPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->DSPDT) && $val->DSPDT !='' && $val->DSPDT !='1900-01-01' ? date('d-m-Y',strtotime($val->DSPDT)):''}}" ></td>
              <td>{{isset($val->DSPNO) && $val->DSPNO !=''?$val->DSPNO:''}}</td>
              <td>{{isset($val->DSPDT) && $val->DSPDT !='' && $val->DSPDT !='1900-01-01' ? date('d-m-Y',strtotime($val->DSPDT)):''}}</td>
