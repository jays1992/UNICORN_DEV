...
@section('content')
    ...
    <div class="container-fluid purchase-order-view filter">     
        <form id="frm_sos_attachment" method="post" enctype="multipart/form-data" action='{{route("transactionmodify",[41,"docuploads"])}}' > 
          @CSRF
        ...
        <input type="hidden" name="VTID_REF" class="form-control" value='{{$objMstVoucherType[0]->VTID}}' />
			...
			<div class="col-lg-2 pl">
         <label style="text-transform:uppercase"> {{$objScheduleOpenSalesOrder->SOSNO}} </label>
        <input type="hidden" name="ATTACH_DOCNO" id="ATTACH_DOCNO" value="{{$objScheduleOpenSalesOrder->SOSID}}" class="form-control" maxlength="50" >
			</div>
			
			<div class="col-lg-1 pl"><p>Document Date</p></div>
			<div class="col-lg-2 pl">
      <label> {{ date("d/m/Y", strtotime($objScheduleOpenSalesOrder->INDATE) )}} </label> 
        <input type="hidden" name="ATTACH_DOCDT" id="ATTACH_DOCDT" value='{{date("Y-m-d", strtotime($objScheduleOpenSalesOrder->INDATE))}}'   />
