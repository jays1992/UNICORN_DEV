        ...
        <div class="multiple table-responsive  ">
            ...
            @php
             $DataStatus= isset($val->STATUS_DESC) && $val->STATUS_DESC!='' ? $val->STATUS_DESC : '';
              ...
              @endphp
                <tr>
                  <td><input type="checkbox" id="chkId{{$val->SEQID}}" value="{{$val->SEQID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}" data-docdate="{{isset($val->ENQDT) && $val->ENQDT !='' && $val->ENQDT !='1900-01-01' ? date('d-m-Y',strtotime($val->ENQDT)):''}}" ></td>
                  <td>{{isset($val->ENQNO) && $val->ENQNO !=''?$val->ENQNO:''}}</td>
                  <td>{{isset($val->ENQDT) && $val->ENQDT !='' && $val->ENQDT !='1900-01-01' ? date('d-m-Y',strtotime($val->ENQDT)):''}}</td>
                  <td>{{isset($val->ENQBY) && $val->ENQBY !=''?$val->ENQBY:''}}</td>
          ...
          var seIdsData = getSeletectedCBox();
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  var viewURL = '{{route("transaction",[35,"view",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("transaction",[35,"attachment",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
                ...
                highlighFocusBtn('activeOk1');
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("transaction",[35,"edit",":rcdId"]) }}';
...
var recordId = soIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("transaction",[35,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("transactionmodify",[35,"cancel"])}}',
                            type:'POST',
