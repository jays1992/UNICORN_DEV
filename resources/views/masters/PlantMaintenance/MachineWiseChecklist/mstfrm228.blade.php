...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->MCKLISTID}}" value="{{$val->MCKLISTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->MACHINE_NO) && $val->MACHINE_NO !=''?$val->MACHINE_NO:''}}</td>
                <td>{{isset($val->MACHINE_DESC) && $val->MACHINE_DESC !=''?$val->MACHINE_DESC:''}}</td>
                <td>{{isset($val->CHECKLIST_NO) && $val->CHECKLIST_NO !=''?$val->CHECKLIST_NO:''}}</td>
                <td>{{isset($val->CHECKLIST_DESC) && $val->CHECKLIST_DESC !=''?$val->CHECKLIST_DESC:''}}</td>
        ...
        <form id="masterForm228Print" action="{{ route('mastergetlist',[228,'printdata'])}}" method="POST" >
            @csrf
      ...
      if(seletedRecord==0){
        ...
        var recordId = resultIdsData[0];
            ...
            if(is_approve==0){
              ...
              var editURL = '{{route("master",[228,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[228,"view",":rcdId"]) }}';
            ...
            if(seletedRecord==0){
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  var recordId = resultIdsData[0];
                  var is_approve = $('#chkId'+recordId).data("rcdstatus");
                 var attachmentURL = '{{route("master",[228,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[228,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[228,"cancel"])}}',
                            type:'POST',
