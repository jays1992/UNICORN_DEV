...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->TRANSPORTERID}}" value="{{$val->TRANSPORTERID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
              <td>{{isset($val->TRANSPORTER_CODE) && $val->TRANSPORTER_CODE !=''?$val->TRANSPORTER_CODE:''}}</td>
              <td>{{isset($val->TRANSPORTER_NAME) && $val->TRANSPORTER_NAME !=''?$val->TRANSPORTER_NAME:''}}</td>
              <td>{{isset($val->REG_ADD1) && $val->REG_ADD1 !=''?$val->REG_ADD1:''}}</td>
              <td>{{isset($val->CP_NAME) && $val->CP_NAME !=''?$val->CP_NAME:''}}</td>
        ...
        <form id="masterForm34Print" action="{{ route('mastergetlist',[34,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[34,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[34,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[34,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[34,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[34,"cancel"])}}',
                            type:'POST',
