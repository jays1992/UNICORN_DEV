  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->SCHEMEID}}" value="{{$val->SCHEMEID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->SCHEME_NO) && $val->SCHEME_NO !=''?$val->SCHEME_NO:''}}</td>
          <td>{{isset($val->SCHEME_DATE) && $val->SCHEME_DATE !=''?$val->SCHEME_DATE:''}}</td>  
          <td>{{isset($val->SCHEME_NAME) && $val->SCHEME_NAME !=''?$val->SCHEME_NAME:''}}</td>  
          <td>{{isset($val->SCHEME_TYPE) && $val->SCHEME_TYPE !=''?$val->SCHEME_TYPE:''}}</td>
          <td>{{isset($val->EFF_FROM_DATE) && $val->EFF_FROM_DATE !='' && $val->EFF_FROM_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->EFF_FROM_DATE)):''}}</td>
          <td>{{isset($val->EFF_TO_DATE) && $val->EFF_TO_DATE !='' && $val->EFF_TO_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->EFF_TO_DATE)):''}}</td>
