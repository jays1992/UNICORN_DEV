...
@section('content')
    ...
    <div class="container-fluid topnav">
            ...
            <div class="row">
                <div class="col-lg-2">
                <a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Prospect Master</a>
                </div><!--col-2-->
    ...
    <div class="container-fluid purchase-order-view filter">     
        <form id="frm_calculation_attachment" method="post" enctype="multipart/form-data" action='{{route("mastermodify",[$FormId,"docuploads"])}}' > 
          @CSRF
...
?>
        ...
        <input type="hidden" name="VTID_REF" class="form-control" value='{{$objMstVoucherType[0]->VTID}}' />
			...
			<div class="col-lg-2 pl">
         <label> {{$objCondition->PCODE}} </label>
        <input type="hidden" name="ATTACH_DOCNO" id="ATTACH_DOCNO" value="{{$objCondition->PID}}" class="form-control" maxlength="50" >
			</div>
			
			<div class="col-lg-1 pl"><p>Document Date</p></div>
			<div class="col-lg-2 pl">
      <label> {{ date("d/m/Y", strtotime($objCondition->INDATE) )}} </label> 
        <input type="hidden" name="ATTACH_DOCDT" id="ATTACH_DOCDT" value='{{date("Y-m-d", strtotime($objCondition->INDATE))}}'   />
              ...
              <th width="15%">View / Download</th>
						...
						<tbody>
                            ...
                            <tr  class="participantRow">
                              <td>{{isset($row->FILESNAME)?$row->FILESNAME:''}}</td>
                              <td >{{$row->REMARKS}}</td>
                              @if($viewfile==true)
