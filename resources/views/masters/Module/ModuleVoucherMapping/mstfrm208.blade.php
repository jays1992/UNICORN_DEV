...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->VTID}}" value="{{$val->VTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->MODULECODE) && $val->MODULECODE !=''?$val->MODULECODE:''}}</td>
                <td>{{isset($val->MODULENAME) && $val->MODULENAME !=''?$val->MODULENAME:''}}</td>
                <td>{{isset($val->VCODE) && $val->VCODE !=''?$val->VCODE:''}}</td>
                <td>{{isset($val->DESCRIPTIONS) && $val->DESCRIPTIONS !=''?$val->DESCRIPTIONS:''}}</td>
                <td>{{isset($val->HEADING) && $val->HEADING !=''?$val->HEADING:''}}</td>
        ...
        <form id="masterForm208Print" action="{{ route('mastergetlist',[208,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[208,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[208,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[208,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[208,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[208,"cancel"])}}',
                            type:'POST',
