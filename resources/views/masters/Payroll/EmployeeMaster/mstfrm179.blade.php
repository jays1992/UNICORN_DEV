...
<div class="container-fluid topnav">
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->EMPID}}" value="{{$val->EMPID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->EMPCODE) && $val->EMPCODE !=''?$val->EMPCODE:''}}</td>
                <td>{{isset($val->FNAME) && $val->FNAME !=''?$val->FNAME:''}}</td>
                <td>{{isset($val->MNAME) && $val->MNAME !=''?$val->MNAME:''}}</td>
                <td>{{isset($val->LNAME) && $val->LNAME !=''?$val->LNAME:''}}</td>
