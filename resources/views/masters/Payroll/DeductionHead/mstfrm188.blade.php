...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->DEDUCTION_HEADID}}" value="{{$val->DEDUCTION_HEADID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->DEDUCTION_HEADCODE) && $val->DEDUCTION_HEADCODE !=''?$val->DEDUCTION_HEADCODE:''}}</td>
                <td>{{isset($val->DEDUCTION_HEAD_DESC) && $val->DEDUCTION_HEAD_DESC !=''?$val->DEDUCTION_HEAD_DESC:''}}</td>
                <td>{{isset($val->DEDUCTION_TYPE_DESC) && $val->DEDUCTION_TYPE_DESC !=''?$val->DEDUCTION_TYPE_DESC:''}}</td>
        ...
        <form id="masterForm188Print" action="{{ route('mastergetlist',[188,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[188,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[188,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[188,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[188,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[188,"cancel"])}}',
                            type:'POST',
