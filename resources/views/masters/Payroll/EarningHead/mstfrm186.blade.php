...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->EARNING_HEADID}}" value="{{$val->EARNING_HEADID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->EARNING_HEADCODE) && $val->EARNING_HEADCODE !=''?$val->EARNING_HEADCODE:''}}</td>
                <td>{{isset($val->EARNING_HEAD_DESC) && $val->EARNING_HEAD_DESC !=''?$val->EARNING_HEAD_DESC:''}}</td>
                <td>{{isset($val->EARNING_TYPE_DESC) && $val->EARNING_TYPE_DESC !=''?$val->EARNING_TYPE_DESC:''}}</td>
        ...
        <form id="masterForm186Print" action="{{ route('mastergetlist',[186,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[186,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[186,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[186,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[186,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[186,"cancel"])}}',
                            type:'POST',
