...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->OTID}}" value="{{$val->OTID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->OT_CODE) && $val->OT_CODE !=''?$val->OT_CODE:''}}</td>
                <td>{{isset($val->OT_DESC) && $val->OT_DESC !=''?$val->OT_DESC:''}}</td>
                <td>{{isset($val->OT_RATE) && $val->OT_RATE !=''?$val->OT_RATE:''}}</td>
                <td>{{isset($val->OT_FACTOR) && $val->OT_FACTOR !=''?$val->OT_FACTOR:''}}</td>
        ...
        <form id="masterForm193Print" action="{{ route('mastergetlist',[193,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[193,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[193,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[193,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[193,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[193,"cancel"])}}',
                            type:'POST',
