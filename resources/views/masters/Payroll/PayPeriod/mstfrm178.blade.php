...
@section('content')
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->PAYPERIODID}}" value="{{$val->PAYPERIODID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                ...
                <td>{{isset($val->MTCODE) && $val->MTCODE !=''?$val->MTCODE:''}} - {{isset($val->MTDESCRIPTION) && $val->MTDESCRIPTION !=''?$val->MTDESCRIPTION:''}}</td>
                <td>{{isset($val->YRCODE) && $val->YRCODE !=''?$val->YRCODE:''}} - ( {{isset($val->YRDESCRIPTION) && $val->YRDESCRIPTION !=''?$val->YRDESCRIPTION:''}} )</td>
        ...
        <form id="masterForm178Print" action="{{ route('mastergetlist',[178,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[178,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[178,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[178,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[178,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[178,"cancel"])}}',
                            type:'POST',
