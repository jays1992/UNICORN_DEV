...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Entitlement Master</a></div>
  ...
  <div class="multiple table-responsive  ">
    ...
    <thead id="thead1">
      ...
      if($val->MNAME){
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->ENTITLEMENT_ID}}" value="{{$val->ENTITLEMENT_ID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->ENTITLEMENT_NO) && $val->ENTITLEMENT_NO !=''?$val->ENTITLEMENT_NO:''}}</td>
          <td>{{isset($val->ENTITLEMENT_DT) && $val->ENTITLEMENT_DT !=''?$val->ENTITLEMENT_DT:''}}</td>
          <td>{{isset($val->EMPCODE) && $val->EMPCODE !=''?$val->EMPCODE:''}}</td>
          <td>{{isset($EMPLOYEE_NAME) && $EMPLOYEE_NAME !=''?$EMPLOYEE_NAME:''}}</td>
