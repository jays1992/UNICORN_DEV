    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive">
           ...
           if(!Empty($val->STATUS) && $val->STATUS=="A"){ 
            ...
            <tr>
                <td><input type="checkbox" id="chkId{{$val->FYID_REF}}" value="{{$val->FYID_REF}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->PAY_PERIOD_CODE) && $val->PAY_PERIOD_CODE !=''?$val->PAY_PERIOD_CODE:''}}</td>
                <td>{{isset($val->PAY_PERIOD_DESC) && $val->PAY_PERIOD_DESC !='' && $val->PAY_PERIOD_DESC !='1900-01-01' ? date('d-m-Y',strtotime($val->PAY_PERIOD_DESC)):''}}</td>
