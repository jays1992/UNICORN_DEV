...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <div class="col-lg-10 topnav-pd">
                  <a href="{{route('master',[155,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->EMPCODEDEFIID}}" value="{{$val->EMPCODEDEFIID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->MANUAL_SR) && $val->MANUAL_SR==1?'Yes':''}}</td>
                <td>{{isset($val->SYSTEM_GRSR) && $val->SYSTEM_GRSR==1?'Yes':''}}</td>
                <td>{{isset($val->INDATE) && $val->INDATE !='' && $val->INDATE !='1900-01-01' ? date('d-m-Y',strtotime($val->INDATE)):''}}</td>
                <td>{{isset($val->DEACTIVATE) && $val->DEACTIVATE == 1 ? 'Yes' : 'No'}}</td>
                <td>{{isset($val->DODEACTIVATE) && $val->DODEACTIVATE !='' && $val->DODEACTIVATE !='1900-01-01' ? date('d-m-Y',strtotime($val->DODEACTIVATE)):''}}</td>
                <td>{{$DataStatus}}</td>
            </tr>
            @endforeach 
            @endif
        ...
        <form id="masterForm{{$FormId}}Print" action="{{ route('mastergetlist',[$FormId,'printdata'])}}" method="POST" >
            @csrf
