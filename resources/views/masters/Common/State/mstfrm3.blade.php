...
<div class="container-fluid topnav">
  <div class="row">
      ...
      <div class="col-lg-10 topnav-pd">
                  <a href="{{route('master',[3,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
          ...
          @endphp
          <tr>
            <td><input type="checkbox" id="chkId{{$val->STID}}" value="{{$val->STID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
            <td>{{isset($val->STCODE) && $val->STCODE !=''?$val->STCODE:''}}</td>
            <td>{{isset($val->NAME) && $val->NAME !=''?$val->NAME:''}}</td>
            <td>{{isset($val->STDCODE) && $val->STDCODE !=''?$val->STDCODE:''}}</td>
            <td>{{isset($val->LANG) && $val->LANG !=''?$val->LANG:''}}</td>
            <td>{{isset($val->NEWSC) && $val->NEWSC !=''?$val->NEWSC:''}}</td>
            <td>{{isset($val->CAPITAL) && $val->CAPITAL !=''?$val->CAPITAL:''}}</td>
            <td>{{isset($val->STTYPE) && $val->STTYPE !=''?$val->STTYPE:''}}</td>
        ...
        <form id="masterForm3Print" action="{{ route('mastergetlist',[3,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = countryIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[3,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[3,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[3,"attachment",":rcdId"]) }}';
...
var recordId = countryIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[3,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[3,"cancel"])}}',
                            type:'POST',
