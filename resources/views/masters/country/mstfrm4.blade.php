...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <div class="col-lg-10 topnav-pd">
                  <a href="{{route('master',[4,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            @endphp
            <tr>
              <td><input type="checkbox" id="chkId{{$val->CTRYID}}" value="{{$val->CTRYID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
              <td>{{isset($val->CTRYCODE) && $val->CTRYCODE !=''?$val->CTRYCODE:''}}</td>
              <td>{{isset($val->NAME) && $val->NAME !=''?$val->NAME:''}}</td>
              <td>{{isset($val->ISDCODE) && $val->ISDCODE !=''?$val->ISDCODE:''}}</td>
        ...
        <form id="masterForm4Print" action="{{ route('mastergetlist',[4,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = countryIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[4,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[4,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[4,"attachment",":rcdId"]) }}';
...
var recordId = countryIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[4,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[4,"cancel"])}}',
                            type:'POST',
