...
@section('content')
    ...
    <div class="container-fluid purchase-order-view filter">     
        <form id="frm_state_attachment" method="post" enctype="multipart/form-data" action='{{route("mastermodify",[3,"docuploads"])}}' > 
          @CSRF
        ...
        <input type="hidden" name="ATTACH_DOCNO" id="ATTACH_DOCNO" value="{{$objCountry->STID}}" class="form-control" maxlength="50" >
			</div>
			
			<div class="col-lg-1 pl"><p>Document Date</p></div>
			<div class="col-lg-2 pl">
      <label> {{ date("d/m/Y", strtotime($objCountry->INDATE) )}} </label> 
        <input type="hidden" name="ATTACH_DOCDT" id="ATTACH_DOCDT" value='{{date("Y-m-d", strtotime($objCountry->INDATE))}}'   />
						...
						<tbody>
                            @foreach($objAttachments as $row)
                                ...
                                <tr  class="participantRow">
                                    <td>{{isset($custFileName[1])?$custFileName[1]:$row->FILESNAME}}</td>
                                    <td >{{$row->REMARKS}}</td>
                                </tr>
                            @endforeach
                        </tbody>    
