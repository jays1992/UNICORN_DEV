  ...
  <div class="multiple table-responsive  ">
    <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->BUGID}}" value="{{$val->BUGID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->BUG_NO) && $val->BUG_NO !=''?$val->BUG_NO:''}}</td>
          <td>{{isset($val->BUG_DATE) && $val->BUG_DATE !='' && $val->BUG_DATE !='1900-01-01' ? date('d-m-Y',strtotime($val->BUG_DATE)):''}}</td>
