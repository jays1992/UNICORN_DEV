    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <div class="col-lg-10 topnav-pd">
                  <a href="{{route('master',[140,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            <tbody> 
                ...
                @endphp
                <tr>
                  <td><input type="checkbox" id="chkId{{$val->CRCOID}}" value="{{$val->CRCOID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                  <td>{{isset($val->EFFDATE) && $val->EFFDATE !='' && $val->EFFDATE !='1900-01-01' ? date('d-m-Y',strtotime($val->EFFDATE)):''}}</td>
                  <!--<td>{{isset($val->ENDDATE) && $val->ENDDATE !='' && $val->ENDDATE !='1900-01-01' ? date('d-m-Y',strtotime($val->ENDDATE)):''}}</td>-->
                  <td>{{isset($val->FRAMOUNT) && $val->FRAMOUNT !=''?$val->FRAMOUNT:''}}</td>
                  <td>{{isset($val->TOAMOUNT) && $val->TOAMOUNT !=''?$val->TOAMOUNT:''}}</td>
        ...
        <form id="masterForm140Print" action="{{ route('mastergetlist',[140,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[140,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[140,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[140,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[140,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[140,"cancel"])}}',
                            type:'POST',
