...
@section('content')
    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <div class="col-lg-10 topnav-pd">
                  <a href="{{route('master',[141,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        ...
        <table id="listingmst" class="display nowrap table table-striped table-bordered" width="100%">
            ...
            @endphp
            <tr>
                <td><input type="checkbox" id="chkId{{$val->BID}}" value="{{$val->BID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                <td>{{isset($val->BCODE) && $val->BCODE !=''?$val->BCODE:''}}</td>
                <td>{{isset($val->NAME) && $val->NAME !=''?$val->NAME:''}}</td>
                <td>{{isset($val->BRANCH) && $val->BRANCH !=''?$val->BRANCH:''}}</td>
                <td>{{isset($val->IFSC) && $val->IFSC !=''?$val->IFSC:''}}</td>
                <td>{{isset($val->ACTYPE) && $val->ACTYPE !=''?$val->ACTYPE:''}}</td>
                <td>{{isset($val->ACNO) && $val->ACNO !=''?$val->ACNO:''}}</td>
        ...
        <form id="masterForm141Print" action="{{ route('mastergetlist',[141,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[141,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[141,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[141,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[141,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[141,"cancel"])}}',
                            type:'POST',
