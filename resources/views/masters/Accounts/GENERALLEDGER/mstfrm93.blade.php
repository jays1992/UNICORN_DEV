    ...
    <div class="container-fluid topnav">
            <div class="row">
                ...
                <div class="col-lg-10 topnav-pd">
                  <a href="{{route('master',[93,'add'])}}" id="btnSelectedRows" class="btn topnavbt" {{isset($objRights->ADD) && $objRights->ADD != 1 ? 'disabled' : ''}}><i class="fa fa-plus"></i> Add</a>
    ...
    <div class="container-fluid purchase-order-view">
        <div class="multiple table-responsive  ">
            ...
            <tbody> 
                ...
                @endphp
                <tr>
                  <td><input type="checkbox" id="chkId{{$val->GLID}}" value="{{$val->GLID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
                  <td>{{isset($val->GLCODE) && $val->GLCODE !=''?$val->GLCODE:''}}</td>
                  <td>{{isset($val->GLNAME) && $val->GLNAME !=''?$val->GLNAME:''}}</td>
                  <td>{{isset($val->ASGNAME) && $val->ASGNAME !=''?$val->ASGNAME:''}}</td>
                  <td>{{isset($val->AGNAME) && $val->AGNAME !=''?$val->AGNAME:''}}</td>
                  <td>{{isset($val->NOGNAME) && $val->NOGNAME !=''?$val->NOGNAME:''}}</td>
                  <td>{{isset($val->NOG_TYPE) && $val->NOG_TYPE !=''?$val->NOG_TYPE:''}}</td>
                  <!-- <td>{{isset($val->ALIAS) && $val->ALIAS !=''?$val->ALIAS:''}}</td> -->
        ...
        <form id="masterForm93Print" action="{{ route('mastergetlist',[93,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[93,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[93,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[93,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[93,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[93,"cancel"])}}',
                            type:'POST',
