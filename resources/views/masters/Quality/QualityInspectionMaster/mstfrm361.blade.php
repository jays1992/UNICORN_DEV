...
<div class="container-fluid topnav">
  <div class="row">
    <div class="col-lg-2"><a href="{{route('master',[$FormId,'index'])}}" class="btn singlebt">Quality Inspection Master</a></div>
  ...
  <div class="multiple table-responsive  ">
    ...
    <thead id="thead1">
      ...
      <tbody> 
        ...
        @endphp
        <tr>
          <td><input type="checkbox" id="chkId{{$val->QICID}}" value="{{$val->QICID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
          <td>{{isset($val->QICNO) && $val->QICNO !=''?$val->QICNO:''}}</td>
          <td>{{isset($val->QICDT) && $val->QICDT !='' && $val->QICDT !='1900-01-01' ? date('d-m-Y',strtotime($val->QICDT)):''}}</td>
  ...
  $('#btnAmendment').on('click', function() {
    ...
    if(seletedRecord==0){
      ...
      if(is_approve==1){
        ...
        var editURL = '{{route("master",[$FormId,"amendment",":rcdId"]) }}';
            editURL = editURL.replace(":rcdId",recordId);
            window.location.href=editURL;
      }else if(is_approve==2){
