...
@section('content')
	...
	@endphp
	<tr>
		<td><input type="checkbox" id="chkId{{$val->RRID}}" value="{{$val->RRID}}" class="js-selectall1" data-rcdstatus="{{$app_status}}"></td>
		<td>{{isset($val->RR_CODE) && $val->RR_CODE !=''?$val->RR_CODE:''}}</td>
		<td>{{isset($val->RR_DESC) && $val->RR_DESC !=''?$val->RR_DESC:''}}</td>
	...
	@endif
        ...
        <form id="masterForm136Print" action="{{ route('mastergetlist',[136,'printdata'])}}" method="POST" >
            @csrf
            ...
            if(seletedRecord==0){
              ...
              var recordId = resultIdsData[0];
                  ...
                  if(is_approve==0){
                    ...
                    var editURL = '{{route("master",[136,"edit",":rcdId"]) }}';
    ...
    $('#btnView').on('click', function() {
      ...
      if(seletedRecord==0){
          ...
          $("#OkBtn1").focus();
            ...
            var viewURL = '{{route("master",[136,"view",":rcdId"]) }}';
              ...
              $("#OkBtn1").focus();
                 ...
                 $("#OkBtn1").focus();
                  ...
                  if(is_approve==2){
                    ...
                    var attachmentURL = '{{route("master",[136,"attachment",":rcdId"]) }}';
...
var recordId = resultIdsDataID;
            $.ajaxSetup({
              ...
              $.ajax({
                  url:'{{ route("master",[136,"MultiApprove"])}}',
                  type:'POST',
                  dataType: 'json',
...
event.preventDefault();
            ...
            $.ajaxSetup({
                          ...
                          $.ajax({
                            url:'{{ route("mastermodify",[136,"cancel"])}}',
                            type:'POST',
