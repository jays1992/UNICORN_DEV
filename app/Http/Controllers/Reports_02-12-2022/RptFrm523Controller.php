...
class RptFrm523Controller extends Controller
{
    protected $form_id = 523;
    protected $vtid_ref   = 593;  //voucher type id
    // //validation messages
    // 
    /**
    ...
    public function index(){    
        ...
        ->select('TBL_MST_ACCOUNTGROUP.AGID AS AGID','TBL_MST_ACCOUNTGROUP.AGCODE AS AGCODE','TBL_MST_ACCOUNTGROUP.AGNAME AS AGNAME')
        ->distinct('TBL_MST_ACCOUNTGROUP.AGID')
        ->get();
		
		$ObjAccountSubGroup = DB::table('TBL_MST_ACCOUNTSUBGROUP')
        ->where('TBL_MST_ACCOUNTSUBGROUP.CYID_REF','=',Auth::user()->CYID_REF)
        ->where('TBL_MST_ACCOUNTSUBGROUP.BRID_REF','=',Auth::user()->BRID_REF)
        ->where('TBL_MST_ACCOUNTSUBGROUP.STATUS','=','A')
        ->select('TBL_MST_ACCOUNTSUBGROUP.ASGID','TBL_MST_ACCOUNTSUBGROUP.ASGCODE','TBL_MST_ACCOUNTSUBGROUP.ASGNAME')
        ->distinct('TBL_MST_ACCOUNTSUBGROUP.ASGID')
        ->get();
        


        return view('reports.Accounts.PNL_MonthWise.rptfrm523',compact(['objRights','objBranchGroup','objBranch','ObjGeneralLedger','ObjAccountGroup',
		'ObjAccountSubGroup']));        
    }  
   ...
   public function ViewReport($request) {
    ...
    else
    {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/PNL_MonthWise');
