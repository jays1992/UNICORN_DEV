...
class TrnFrm486Controller extends Controller{
    ...
    public function showRecord($id,$type,$ActionStatus){
        ...
        $FANO = "APPROVAL".$objFinalAppr[0]->FA_NO;
            ...
            return view($this->view.$FormId.$type,compact(['objResponse','MAT','tasktype','conctprsn','priorty','leadstatus','leadsource','resp','activitytype','country','emplyee',
            'MATSITE','MATNEWCALL','NEWTASKS','PRODUCTDETAILS','objDataList','objRights','ActionStatus','FormId','design','objResp','objCustProspt']));
        }
    }
    ...
    public function updateRecord($request,$type){
        ...
        $REMARKS                =   trim($request['REMARKS'])?$request['REMARKS']:NULL;
        $DEALERID_REF           =   trim($request['DEALERIDREF'])?trim($request['DEALERIDREF']):0;
        $REASONOF_CLOSURE       =   trim($request['REASONOF_CLOSURE'])?$request['REASONOF_CLOSURE']:NULL;
        $USERID_IDREF           =   trim($request['USERID_IDREF'])?$request['USERID_IDREF']:NULL;
        $LEAD_FINALSTATUS       =   trim($request['LEAD_FINALSTATUS'])?trim($request['LEAD_FINALSTATUS']):NULL;
