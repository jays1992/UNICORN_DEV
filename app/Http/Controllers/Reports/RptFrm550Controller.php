...
//use App\Exports\UserRoleListingReport;
...
class RptFrm550Controller extends Controller
{
    ...
    protected $view     = "reports.Payroll.UserRoleListingReport.rptfrm";
    ...
    public function index(){   
        ...
        $ObjModule = DB::table('TBL_MST_MODULE')
        //->where('TBL_MST_MODULE.CYID_REF','=',Auth::user()->CYID_REF)
        //->where('TBL_MST_MODULE.BRID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_MODULE.STATUS','=','A')
        ->where('TBL_MST_MODULE.DEACTIVATED','=','0')
        ->where('TBL_MST_MODULE.DODEACTIVATED','=',NULL)
        ->select('TBL_MST_MODULE.MODULEID','TBL_MST_MODULE.MODULECODE','TBL_MST_MODULE.MODULENAME')
        ->distinct('TBL_MST_MODULE.MODULEID')
        ->get();

        //dd($ObjModule);

        $ObjUser = DB::table('TBL_MST_USER')
        //->where('TBL_MST_USER.CYID_REF','=',Session::get('BRID_REF'))
        ->where('TBL_MST_USER.STATUS','=','A')
        ->where('TBL_MST_USER.DEACTIVATED','=','0')
        ->where('TBL_MST_USER.DODEACTIVATED','=',NULL)
        ->select('TBL_MST_USER.USERID','TBL_MST_USER.UCODE','TBL_MST_USER.DESCRIPTIONS')
        ->distinct('TBL_MST_USER.USERID')
        ->get();   
        ...
        //dd($company_check); 
        return view($this->view.$FormId,compact(['objRights','objBranchGroup','objBranch','ObjModule','ObjUser','company_check','FormId']));     
    }  
    ...
    public function ViewReport($request) {
        ...
        $result = $ssrs->loadReport(Session::get('ssrs_config')['INSTANCE_NAME'].'/User_Role_Listing_Report');
        ...
        {
            //return Excel::download(new UserRoleListingReport($MODULENAME,$USERNAME,$From_Date,$To_Date,$BranchGroup,$BranchName,$CYID_REF), 'UserRoleListingReport.xlsx');
