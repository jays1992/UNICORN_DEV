...
class SalesReturnSummary implements FromCollection, WithHeadings
{
 ...
 function __construct($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID) {
    ...
    */
    public function collection()
    {
      ...
      return collect( $data=DB::select("SELECT TBL_TRN_SLSR01_HDR.SRNO,
      ...
      C.SAP_CUSTOMER_NAME,
	  CL.NAME, 
      SUM(TBL_TRN_SLSR01_MAT.SRQTY*TBL_TRN_SLSR01_MAT.SRRATE) AS GROSS_AMOUNT,
      SUM(((TBL_TRN_SLSR01_MAT.SRQTY*TBL_TRN_SLSR01_MAT.SRRATE)+(case when TBL_TRN_SLSR01_MAT.CGST IS not NULL then convert(numeric(14,2),((TBL_TRN_SLSR01_MAT.SRQTY*TBL_TRN_SLSR01_MAT.SRRATE*TBL_TRN_SLSR01_MAT.CGST)/100)) else  0 end)+
   ...
   WHERE
      (TBL_TRN_SLSR01_HDR.STATUS = 'A') AND 
      ...
      (TBL_TRN_SLSR01_MAT.ITEMID_REF	in ( $ITEMID))
	   ...
	   GROUP BY TBL_TRN_SLSR01_HDR.SRNO,
      TBL_TRN_SLSR01_HDR.SRDT,
      BG.BG_DESC,
      BR.BRNAME,
      C.CCODE, 
