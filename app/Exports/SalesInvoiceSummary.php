...
class SalesInvoiceSummary implements FromCollection, WithHeadings
{
 ...
 function __construct($SGLID,$From_Date,$To_Date,$BranchGroup,$BranchName,$ITEMGID,$ITEMID,$STATUS,$CYID_REF) {
    ...
    */
    public function collection()
    {
      ...
      return collect( $data=DB::select("SELECT TBL_TRN_SLSI01_HDR.SINO,
      TBL_TRN_SLSI01_HDR.SIDT,
      BG.BG_DESC AS BRANCH_GROUP,
      BR.BRNAME,
      TBL_MST_CUSTOMER.CCODE AS CUSTOMER_CODE,
      TBL_MST_CUSTOMER.NAME AS CUSTOMER_NAME, 
      TBL_MST_CUSTOMER.SAP_CUSTOMER_CODE,
      TBL_MST_CUSTOMER.SAP_CUSTOMER_NAME,
      CL.NAME AS BILLTO, 
      SUM(TBL_TRN_SLSI01_MAT.RATEPUOM*TBL_TRN_SLSI01_MAT.SIMAIN_QTY) AS  GROSS_AMOUNT,
      SUM(((TBL_TRN_SLSI01_MAT.SIMAIN_QTY*TBL_TRN_SLSI01_MAT.RATEPUOM)+(case when TBL_TRN_SLSI01_MAT.CGST IS not NULL then convert(numeric(14,2),((TBL_TRN_SLSI01_MAT.SIMAIN_QTY*TBL_TRN_SLSI01_MAT.RATEPUOM*TBL_TRN_SLSI01_MAT.CGST)/100)) else  0 end)+
      ...
      BG.BG_DESC,
      BR.BRNAME,
      TBL_MST_CUSTOMER.CCODE,
      TBL_MST_CUSTOMER.NAME, 
      TBL_MST_CUSTOMER.SAP_CUSTOMER_CODE,
      TBL_MST_CUSTOMER.SAP_CUSTOMER_NAME,
      CL.NAME, 
      TBL_TRN_SLSI01_HDR.STATUS"));
