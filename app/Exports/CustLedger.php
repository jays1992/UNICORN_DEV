...
class CustLedger implements FromCollection, WithHeadings
{
 ...
 function __construct($SGLID,$CUSTOMERGROUP,$From_Date,$To_Date,$BranchGroup,$BranchName,$CYID) {
    ...
    */
    public function collection()
    {
      ...
      return collect( $data=DB::select("SELECT BG.BG_DESC AS BRANCH_GROUP,
       BR.BRNAME,
	   C.CCODE,
       C.NAME,
       C.SAP_CUSTOMER_CODE,C.SAP_CUSTOMER_NAME,
      DBO.FN_CODBL(A.GLID_REF,'$this->From_Date') AS OPENING_DR,
      DBO.FN_COCBL(A.GLID_REF,'$this->From_Date') AS OPENING_CR, 
	  SUM(ISNULL(A.DR_AMT,0)) AS DR_AMT,
      SUM(ISNULL(A.CR_AMT,0)) AS CR_AMT,
	  IIF((DBO.FN_CODBL(A.GLID_REF,'$this->From_Date')-DBO.FN_COCBL(A.GLID_REF,'$this->From_Date')+SUM(ISNULL(A.DR_AMT,0))-SUM(ISNULL(A.CR_AMT,0)))>0,
      (DBO.FN_CODBL(A.GLID_REF,'$this->From_Date')-DBO.FN_COCBL(A.GLID_REF,'$this->From_Date')+SUM(ISNULL(A.DR_AMT,0))-SUM(ISNULL(A.CR_AMT,0))),0) AS CLOSING_DR,
      IIF((DBO.FN_CODBL(A.GLID_REF,'$this->From_Date')-DBO.FN_COCBL(A.GLID_REF,'$this->From_Date')+SUM(ISNULL(A.DR_AMT,0))-SUM(ISNULL(A.CR_AMT,0)))<0,
      ABS((DBO.FN_CODBL(A.GLID_REF,'$this->From_Date')-DBO.FN_COCBL(A.GLID_REF,'$this->From_Date')+SUM(ISNULL(A.DR_AMT,0))-SUM(ISNULL(A.CR_AMT,0)))),0) AS CLOSING_CR
FROM (
...
WHERE A.SGLID_REF='S' 
AND (H.SOURCE_DOCTYPE IN ('AR DEBIT CREDIT NOTE','SALES INVOICE','SALES SERVICE INVOICE','SALES RETURN','CSV','RECEIPT') OR H.SOURCE_DOCTYPE IS NULL)
...
AND (SOURCE_DOCTYPE IN  ('AR DEBIT CREDIT NOTE','SALES INVOICE','SALES SERVICE INVOICE','SALES RETURN','CSV','RECEIPT')
...
INNER JOIN TBL_MST_CUSTOMER C ON A.GLID_REF=C.SLID_REF
INNER JOIN TBL_MST_CUSTOMERGROUP CG ON C.CGID_REF = CG.CGID
...
WHERE A.GLID_REF in ($SGLID) and C.Status = 'A' and (C.DEACTIVATED = 0 or C.DEACTIVATED IS NULL)
and C.CYID_REF = $this->CYID and A.BRID_REF IN ($BranchName) AND C.CGID_REF in ($CUSTOMERGROUP)
GROUP BY C.CCODE,C.NAME,C.SAP_CUSTOMER_CODE,C.SAP_CUSTOMER_NAME,BG.BG_DESC,BR.BRNAME,A.GLID_REF
"));
